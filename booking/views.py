...
def class_detail(request, pk):
    ...
    current_participants = Booking.objects.filter(
        crossfit_class=crossfit_class).count()

    if request.user.is_authenticated:
        is_booked = Booking.objects.filter(
            user=request.user, crossfit_class=crossfit_class).exists()
...
def book_class(request, pk):
    crossfit_class = get_object_or_404(CrossfitClass, pk=pk)
    if not Booking.objects.filter(user=request.user, crossfit_class=crossfit_class
    ).exists():
        Booking.objects.create(
            user=request.user, crossfit_class=crossfit_class)
        return redirect('thank_you')  # Redirect to a thank-you after booking
...
def delete_class(request, pk):
    ...
    return render(request, 'booking/delete_class.html', {
        'crossfit_class': crossfit_class})
